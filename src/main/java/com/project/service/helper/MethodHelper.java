...
public class MethodHelper {
    ...
    //isUserExist  -->user'in db'de olup olmadigi kontrolunu surekli yaptigimiz icin bu methodu helper packageda olusturduk
    ...
    // Rol kontrolu yapan method--> id'si verilen id teacher mi gibi
    public void checkRole(User user, RoleType roleType){
        if (!user.getUserRole().getRoleType().equals(roleType)){
            throw new ResourceNotFoundException(String.format(ErrorMessages.NOT_FOUND_USER_WITH_ROLE_MESSAGE, user.getId(), roleType));
        }
    }
    ...
    //findByUsernameEquals() methodu User donduruyor, Optional User dondurmuyor bu yuzden orElseThrow diyemeyiz, bunu Optinal ise diyeblrz
        if (user.getId()==null){//id null ise o user yok demektir
