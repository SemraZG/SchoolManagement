...
public interface ContactMessageRepository extends JpaRepository<ContactMessage, Long> {
    ...
    @Query("SELECT c FROM ContactMessage c WHERE FUNCTION('DATE', c.dateTime) BETWEEN ?1 AND ?2")//bu methodu ile hangi querynin calisilacagi soylenecek demek
    List<ContactMessage> findMessagesBetweenDates(LocalDate beginDate, LocalDate endDate);//ContactMessage entity icindeki dateTime icindeki date aldik sadece
